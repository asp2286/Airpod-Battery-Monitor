'use strict';
const {Gio, GLib, GObject, GnomeBluetooth} = imports.gi;
const ExtensionUtils = imports.misc.extensionUtils;
const Me = ExtensionUtils.getCurrentExtension();

const {BluezDeviceProxy} = Me.imports.lib.dbus;
import {ModelList = Me.imports.lib.devices.js;
...
var BluetoothClient = GObject.registerClass({
    ...
    constructor(extensionObj, settings) {
        ...
        this._client.connectObject(
            'device-added', (c, device) => {
                this._connectDeviceNotify(device);
                this._sync();
            },
            'device-removed', (c, path) => {
    ...
    *getDevices() {
    ...
    _checkForAirPods(path) {
        ...
        if (isAirpod && devProperties.Modalias) {
            const regex = /v004Cp([0-9A-Fa-f]{4})d/;
            const match = devProperties.Modalias.match(regex);
            if (match) {
                const detectedModel = match[1].toUpperCase();
                const correctedModel = detectedModel.substring(2) + detectedModel.substring(0, 2);
                if (ModelList.some(entry => entry.key === correctedModel))
    ...
    _updateGsettingDevices(path, dev, isAirpod) {
        ...
        if (existingPathIndex === -1) {
            pairedAirpods.push(JSON.stringify({path, alias: dev.alias, model: isAirpod, connected: dev.connected, paired: dev.paired}));
            arrayUpdated = true;
        } else {
            ...
            const model = existingItem['model'];
            if (model !== isAirpod && !ModelList.some(entry => entry.key === model)) {
                existingItem['model'] = isAirpod;
                arrayUpdated = true;
            }
        ...
        for (const dev of devices) {
            ...
            if (this._pairedDevices.has(path)) {
                ...
                if (isAirpod) {
                    if (dev.connected)
                        connectedAirpods++;
                    this._updateGsettingDevices(path, dev, isAirpod);
                }
                continue;
            }
            if (this._removedDeviceList.length > 0) {
            ...
            const isAirpod = this._checkForAirPods(path);
    ...
    destroy() {
        ...
        this._airpodService = null;
        if (this._devicesChangedId)
            GLib.source_remove(this._devicesChangedId);
        this._devicesChangedId = null;
