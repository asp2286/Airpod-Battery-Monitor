...
import {AirpodIndicators, AirpodTray} from './airpodTrayIndicator.js';
...
class AirpodService extends GObject.Object {
    constructor(extensionObj, settings) {
        ...
        this._settings.connectObject(
            ...
            'changed::enable-indicator', () => this._reloadGUI(),
            'changed::enable-panel-menu', () => this._reloadGUI(),
            'changed::enable-message-tray', () => this._reloadGUI(),
            this
        );
    }
    ...
    _updateAirpodList() {
        ...
        for (const airpod of airpodList) {
            const {path, model, connected} = airpod;
    ...
    _startGUI() {
        ...
        const indicatorEnabled = this._settings.get_boolean('enable-indicator');
        if (indicatorEnabled) {
            if (!this._indicator)
                this._indicator = new AirpodIndicators(this._settings, this._extensionObj, airpods);
            else
        ...
        const panelMenuEnabled = this._settings.get_boolean('enable-panel-menu');
        if (panelMenuEnabled) {
            if (!this._panel) {
        ...
        const messageTrayEnabled = this._settings.get_boolean('enable-message-tray');
        if (messageTrayEnabled) {
            if (!this._messageTray)
                this._messageTray = new AirpodTray(this._settings, this._extensionObj, airpods);
            else
                this._messageTray.updateDevices(airpods);
        } else {
    ...
    _filterAirpods(rawScannedAirpods) {
        ...
        for (let i = 0; i < numberOfConnecedAirpod; i++) {
            ...
            for (let j = 0; j < scannedAirpods.length; j++) {
                const modelValue = scannedAirpods[j].data.substring(6, 10);
